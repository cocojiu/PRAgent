...
class LlmReviewPromptBuilder {
    ...
    String buildPrompt(ReviewTask task, PullRequestDiff diff, LlmReviewContext context) {
        ...
        return """
            ...
            3. 输出前必须做变更前后反事实判断：问题须由新增/修改行导致，且变更后行为确实更差；无法证明时直接省略，不得用 LOW/MEDIUM confidence 保留猜测。仅当问题本身已被直接证明、但影响范围不确定时才降低 confidence。若候选声称修改造成回归、兼容性下降或替换错误，evidence 必须同时写出被删除/替换的旧行为与新增行的新行为并说明新行为为何更差；没有相关旧行为或上下文不足以完成比较时直接省略。全新可达行为仍可基于明确调用路径报告。语义未变的换行、缩进或对象/参数列表展开不是新增行为；不得把仅因格式重排而出现在新增行中的既有字段或参数当作问题证据或主锚点。
            ...
            8. 审查外部 API/CI 调用时，不得猜测仓库身份、账号可分配性、令牌权限或部署环境会失败。由平台上下文提供的 owner/repo 等标识应按已展示语义判断；若上下文已展示所需权限且当前 diff 未删除或削弱它，不得报告权限不足。只有新增值在当前证据中明确无效，或新增调用存在可证明的失败路径及功能回归时才能报告。 9. 审查 Bash 时必须区分算术命令和算术展开的退出状态：`(( expression ))` 在结果为 0 时返回非零，而 `name=$(( expression ))` 的赋值成功时不以算术结果 0 表示失败；在 `set -e` 脚本中用后者替换计数器更新通常是正确修复。立即保存 `$?`、输出固定诊断并以原状态退出的 ERR trap，与 EXIT cleanup 配合属于常规错误保留；除非当前 diff 可证明它覆盖成功状态、递归触发、泄露敏感值或跳过清理，否则不得报告。
            10. 没有可信问题时返回 riskLevel=INFO 且 findings=[]。 11. 只有能够精确替换当前新增行、且替换范围不超过 5 行时，fixExample 才填写完整替换内容，
